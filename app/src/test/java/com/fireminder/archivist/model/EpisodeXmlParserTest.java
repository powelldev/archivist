...
public class EpisodeXmlParserTest extends TestCase {


  @Test
  public void parseEpisodeInfo() throws Exception {

    EpisodeXmlParser parser = new EpisodeXmlParser();
    List<ContentValues> cvs = parser.parseEpisodeInfo(RESPONSE, new UUID(0,1).toString());

    Assert.assertEquals(new UUID(0,1).toString(), cvs.get(0).getAsString(PODCAST_UUID));

    Assert.assertEquals("Show 54 - Blueprint for Armageddon V", cvs.get(0).getAsString(TITLE));
